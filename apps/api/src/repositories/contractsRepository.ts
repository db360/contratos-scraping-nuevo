...
import type { ContractRecord, ScrapeRunInsert, SourceType } from "../types.js";

export async function upsertContracts(
  contracts: ContractRecord[]
): Promise<ResultSetHeader | { affectedRows: number }> {
  if (!contracts.length) {
...
export async function getContracts({ sourceType, limit, offset }: GetContractsArgs) {
...
export async function countContracts({ sourceType }: { sourceType?: SourceType }) {
  const filters: string[] = [];
  const params: string[] = [];
...
export async function insertScrapeRun(run: ScrapeRunInsert): Promise<void> {
  const sql = `
